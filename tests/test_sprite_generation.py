...
@pytest.mark.parametrize("size", [pytest.param(22), pytest.param(43)])
def test__generate_sprite_for_file_name__circle(size):
    ...
    ImageDraw.Draw(expected).ellipse((0, 0, size - 1, size - 1), outline="black")
    assert_images_equal(result, expected)


@pytest.mark.parametrize(
    "color, size", [pytest.param("red", 25), pytest.param("blue", 199)]
)
def test__generate_sprite_for_file_name__circle_colored(color, size):
    ...
    ImageDraw.Draw(expected).ellipse(
        (0, 0, size - 1, size - 1), fill=color, outline="black"
    )
    assert_images_equal(result, expected)


@pytest.mark.parametrize("width, height", [pytest.param(25, 15), pytest.param(10, 100)])
def test__generate_sprite_for_file_name__ellipse(width, height):
    ...
    ImageDraw.Draw(expected).ellipse((0, 0, width - 1, height - 1), outline="black")
    assert_images_equal(result, expected)


@pytest.mark.parametrize(
    "color, width, height",
    [pytest.param("orange", 5, 10), pytest.param("blue", 66, 55)],
)
...
@pytest.mark.parametrize(
    "width, height", [pytest.param(12, 55), pytest.param(100, 300)]
)
def test__generate_sprite_for_file_name__rect(width, height):
    ...
    expected = make_canvas(width, height)
    ImageDraw.Draw(expected).rectangle((0, 0, width - 1, height - 1), outline="black")
    assert_images_equal(result, expected)


@pytest.mark.parametrize(
    "color, width, height",
    [pytest.param("orange", 51, 4), pytest.param("blue", 305, 511)],
)
